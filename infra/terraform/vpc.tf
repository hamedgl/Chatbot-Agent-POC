...
# ── Public subnets (ALB + ECS Fargate) ────────────────────────────────────────
# ECS tasks get public IPs so they can reach Bedrock/ECR without a NAT Gateway.
# Inbound access is restricted to ALB only via security groups.
...
resource "aws_security_group" "ecs" {
  ...
  description = "ECS tasks: inbound from ALB only, unrestricted outbound"
  vpc_id      = aws_vpc.main.id
